...
%%%------------------------------------------------------------------------
%%%   TriT2WRELM / triangular type-2 Weighted Regularized ELM
%%%------------------------------------------------------------------------
...
if isEXInvalAdd ==1
    ...
    [nn, acc_train{4}]   = elm_train(traindata, trainlabel, nn,alg_ind);
    [nn4, acc_test{4}]   = elm_test(testdata, testlabel, nn, alg_ind); 
end
%%%%%%%%%%%%%%%%%%%%%%
fprintf('      %19s      |      %.4d      |      %.4d      |      %.4d      |      %.4d      \n',...
    nn.method{4},acc_train{4},acc_test{4},nn.time_train(4),cond(nn4.H,2));

%%%------------------------------------------------------------------------
%%%   TriT2RELM / triangular type-2 Regularized ELM
%%%------------------------------------------------------------------------

nn.method{5}         = method{5};
nn.wfun{5}           = '1';
nn.scale_method{5}   = 1;
alg_ind              = 5;
nn.activefunction{5} = 't2sig2';
nn                   = elm_initialization(nn);
nn.C{5}              = C(ci);
if isEXInvalAdd ==1
    [nn, acc_train{5}]   = elm_train(traindata, trainlabel, nn,alg_ind);
    [nn5, acc_test{5}]   = elm_test(testdataEx, testlabelEx, nn, alg_ind);    
else
    [nn, acc_train{5}]   = elm_train(traindata, trainlabel, nn,alg_ind);
    [nn5, acc_test{5}]   = elm_test(testdata, testlabel, nn, alg_ind); 
end
%%%%%%%%%%%%%%%%%%%%%%
fprintf('      %19s      |      %.4d      |      %.4d      |      %.4d      |      %.4d     \n',...
    nn.method{5},acc_train{5},acc_test{5},nn.time_train(5),cond(nn5.H,2));
...
%%%%%%%%%%%%%%%%%%%%%%
fprintf('      %19s      |      %.4d      |      %.4d      |      %.4d      |  \n\n',...
    nn.method{6},acc_train{6},acc_test{6},nn.time_train(6) );
cond_numer=[cond(nn1.H,2),cond(nn2.H,2),cond(nn3.H,2),cond(nn4.H,2),cond(nn5.H,2),0];
TimeRec1=[nn.time_train(1),nn.time_train(2),nn.time_train(3),nn.time_train(4),nn.time_train(5),nn6.time_train(6)];
TimeRec2=[nn1.time_test(1),nn2.time_test(2),nn3.time_test(3),nn4.time_test(4),nn5.time_test(5),nn6.time_test(6)];
